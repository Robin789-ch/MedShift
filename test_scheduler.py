...
class DepartmentSchedulingTest(unittest.TestCase):
    ...
    def test_department_request_is_optimized_in_one_model(self) -> None:
        config = base_config()
    ...
    def test_recovery_and_formation_count_toward_max_hours_but_holiday_does_not(self) -> None:
        config = base_config()
        config["num_employees"] = 1
        config["employee_names"] = ["Ada"]
        config["departments"] = [
            {
                ...
                "symbol": "N",
                "duration_hours": 12,
                "requirements": [1, 1, 1, 1, 0, 0, 0],
            }
        ]
        config["max_hours_per_week"] = 50

        for capped_shift in ("R", "F"):
            with self.subTest(shift=capped_shift):
                capped_config = {**config, "fixed_assignments": [[0, capped_shift, 4]]}
                result, _log = self.solve_quietly(capped_config)
                self.assertIsNone(result)

        holiday_config = {**config, "fixed_assignments": [[0, "H", 4]]}
        result, _log = self.solve_quietly(holiday_config)
        self.assertIsNotNone(result)

    def test_pending_overtime_prefers_recovery_for_that_employee(self) -> None:
        config = base_config()
        config["departments"][0]["requirements"] = [0, 0, 0, 0, 0, 0, 0]
        config["employee_overtime_hours"] = [8, 0]

        result, _log = self.solve_quietly(config)

        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result["plan"][0].count("R"), 1)
        self.assertEqual(result["plan"][1].count("R"), 0)

    def test_no_pending_overtime_discourages_recovery(self) -> None:
        config = base_config()
        config["departments"][0]["requirements"] = [0, 0, 0, 0, 0, 0, 0]
        config["employee_overtime_hours"] = [0, 0]

        result, _log = self.solve_quietly(config)

        self.assertIsNotNone(result)
        assert result is not None
        self.assertEqual(result["plan"][0].count("R"), 0)
        self.assertEqual(result["plan"][1].count("R"), 0)
